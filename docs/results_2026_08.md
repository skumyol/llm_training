...
### 15.2.3 The optimizer must match across pretraining and fine-tuning

Muon wins pretraining, but "Muon to pretrain, AdamW to fine-tune" — the obvious reading of that —
is **wrong**:

| pretrain | fine-tune | vocab | pretrain val ppl | fine-tune val ppl |
|---|---|---|---:|---:|
| AdamW (`C`) | AdamW (`D`) | gpt2-50k | 50.57 | **14.65** |
| Muon (`G`) | Muon (`H`) | gpt2-50k | **49.25** | 15.16 |
| Muon (`G`) | AdamW (`Q`) | gpt2-50k | 49.25 | 19.31 |
| Muon (`I`) | AdamW (`R`) | bpe-16k | 51.29 | 20.97 |

Mismatching the stages costs **4.2 perplexity** — far more than either optimizer choice on its own.
A Muon-trained network sits at weight scales AdamW's per-parameter second moments are not calibrated
for, and the fine-tune spends its budget re-adapting rather than learning the target domain. Matched
Muon (15.16) is close to matched AdamW (14.65) but does not beat it, so **Muon's 1.3-perplexity
pretraining win does not transfer**.

Caveat: `Q` and `R` both peaked at their final epoch (12), so neither had converged; more epochs
would close some of that gap, but not 4.2.

The practical conclusion is unglamorous — the existing AdamW/AdamW recipe is still the best
end-to-end — and the useful one is that pretraining perplexity was the wrong objective to chase.

### 15.2.4 Modern decoder block and longer context (running)

`TinyGPTLM` is the 2019 recipe: learned position embeddings, LayerNorm, a 4× GELU MLP, biases.
`arch: llama` replaces it with RoPE, RMSNorm, SwiGLU sized to 8/3·n_embd so parameters stay level
with the GELU MLP it replaces, QK-norm, and no biases. Parity, causality and RoPE length extension
are asserted by `slm_training/tests/test_llama_block.py` (27.66 M vs 27.42 M, +0.9%).

First-epoch validation perplexity, all on the same 12,358-token validation set:

| run | block | vocab | ctx | params | epoch-1 val ppl |
|---|---|---|---:|---:|---:|
| `slm_C_pretrain` | GPT | 50k | 256 | 44.8 M | 65.17 |
...
context genuinely helps prediction, but part of the gap is that fewer tokens are scored cold. A clean
comparison evaluates every model at one context length; that is not yet done, so P's margin is an
upper bound.

### 15.2.5 Generative structured prediction of the latent state (running)

Every variation tried on the head-based predictor — pooling, context length, sampler, head depth,
head learning rate, label schema, epoch count, counterfactual filtering — lands within a few points
of the same ceiling. The M-series run by the parallel session is the clearest statement of this: all
six configurations score **below** the L3 baseline on matched validation data.

| run | val macro-$F_1$ | Δ vs L3 | val `response_policy` | Δ vs L3 |
|---|---:|---:|---:|---:|
| **L3_meanpool** | **0.5608** | — | **0.5558** | — |
| M2_deephead | 0.5149 | −0.046 | 0.5064 | −0.049 |
| M6_hilr (2× head LR) | 0.5131 | −0.048 | 0.5224 | −0.033 |
| M3_binary | 0.5115 | −0.049 | 0.5731 (2-class) | n/c |
| M1_clean7 | 0.5072 | −0.054 | 0.3626 (7-class) | n/c |
| M4_clean7_deep | 0.5054 | −0.055 | 0.3752 (7-class) | n/c |
| M5_long (16 epochs) | 0.4975 | −0.063 | — | — |

All six share L3's `val_file` and selection metric, so these are directly comparable. Two readings to
avoid: M3's 0.5731 is a **2-class** macro-$F_1$ whose chance floor is 0.50, so it is ~7 points of
signal and does not belong in a column beside 10-class numbers; and M6 is a regression, not an
improvement — its 0.5224 beats L3's *test* 0.4525 but loses to L3's *val* 0.5558, which is the
matched comparison.

What all of these share is the architecture: pool the backbone to one vector, read 29 independent
linear probes off it. Nothing lets `reveal_decision` condition on `secrecy_pressure`, and the
backbone's actual competence — next-token prediction over structured text — is unused.

`src/training/latent_sft.py` serialises the 29 fields as a text block and trains the backbone to emit
it, so the fields are produced autoregressively and condition on each other. Scoring runs through the
same `compute_latent_metrics`, so the numbers stay comparable. A generated block that cannot be
parsed is scored as **wrong** rather than dropped — dropping it would reward a model for declining to
answer — and `parse_rate` and `field_coverage` are reported alongside. Trained on the full split
including counterfactuals, per §15.1.0.
