#!/usr/bin/env bash
#SBATCH --job-name=gemma-baseline
#SBATCH --output=/scratch/%u/logs/gemma_baseline_%j_%x.out
#SBATCH --error=/scratch/%u/logs/gemma_baseline_%j_%x.err
...
python -m src.train.run_gemma_unsloth \
    ...
    2>&1 | tee "${LOG_DIR}/gemma_baseline_${SLURM_JOB_ID:-manual}.log"
