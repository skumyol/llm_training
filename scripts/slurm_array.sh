#!/usr/bin/env bash
# =============================================================================
# slurm_array.sh — SLURM job array: multi-seed + multi-arch training
# =============================================================================
# Each array task trains one (architecture, seed) combo.
# All metrics go to the remote MLflow server for comparison.
#
# Usage:
...
#
# Array mapping for small_lm:
#   TASK 0 = (archs[0], seeds[0]), TASK 1 = (archs[0], seeds[1]), ...
#
# Array mapping for llm full_pipeline:
#   TASK 0 = latent, TASK 1 = response, TASK 2 = joint
# =============================================================================
...
# ── Load env ──────────────────────────────────────────────────────────────────
case "${SYSTEM}" in
    llm) VENV_NAME="${VENV_NAME:-llm_env}" ;;
    slm) VENV_NAME="${VENV_NAME:-slm_env}" ;;
esac

module purge 2>/dev/null || true
module load "${CUDA_MOD:-cuda/12.4.0}" 2>/dev/null || true
module load "${PY_MOD:-python/3.12}" 2>/dev/null || true
source "${WORK_BASE}/venvs/${VENV_NAME}/bin/activate"

# ── Map TASK_ID → (arch, seed) ────────────────────────────────────────────────
...
if [ "${SYSTEM}" = "slm" ] && [ "${STAGE}" = "small_lm" ]; then
    ...
    echo "================================================================"
    echo "  Job Array: ${SLURM_ARRAY_JOB_ID:-local}[${TASK_ID}]"
