...
Conditioning a 1.1B response model on just 8 numbers---a compressed OCEAN+VAD personality vector---reduces perplexity by 12.3\% over an unconditioned baseline that sees identical training data.
...
Automatic checks detect zero secret leakage in sampled generations.
...
(2)~empirical evidence that a compact 8-dimensional personality+affect vector ($<$0.1\% of the model's parameter budget) yields a 12.3\% perplexity reduction over an unconditioned baseline;
...
\textbf{Our contribution beyond this literature:}
We demonstrate that soft-prefix conditioning with an 8-dimensional OCEAN+VAD vector produces a reliable 12.3\% perplexity reduction for NPC dialogue (Table~\ref{tab:response_ppl}).
This is not a marginal gain on a narrow task---it is a systematic improvement on a challenging multi-character, multi-scenario generation problem.
...
Conditioned generation~\cite{li2021prefix,keskar2019ctrl,dathathri2020plug}
    ...
    & Multi-dimensional social-state conditioning (OCEAN+VAD); integrated into a 3-stage pipeline with joint training \\
\midrule
...
\textit{Track D}: per-head accuracy, macro-F1, estimated Cohen's $\kappa$~\cite{landis1977measurement}, per-group means, response policy F1.
...
\textbf{Finding (RQ3):} Just 8 numbers---a compressed OCEAN+VAD vector representing the NPC's personality and emotional state---reduce response perplexity by \textbf{12.3\%} (3.30$\to$2.90; Table~\ref{tab:response_ppl}).
This is surprising because both the conditioned and unconditioned models are fine-tuned on identical training data; the only difference is that the ConditionalDialogue model receives an 8-dimensional prefix vector projected from the encoder outputs.
The conditioning vector constitutes less than 0.1\% of the model's parameter budget (8 values versus 1.1B parameters), yet provides a reliable and reproducible improvement.
...
The 12.3\% perplexity reduction from an 8-dimensional conditioning vector is notable for what it costs: essentially nothing.
The OCEAN+VAD vector is precomputed from lightweight DistilBERT encoders (CPU inference, {$<$}10\,ms) and constitutes 0.007\% of the 1.1B model's input dimensionality.
This suggests that even a crude compressed summary of who the NPC is and how they feel provides the model with a useful prior that reduces uncertainty about response style.
The fact that both models see identical training data rules out the possibility that the gain comes from data rather than representation.
...
\textbf{Second, a tiny conditioning signal produces a reliable improvement.}
Eight numbers---a compressed OCEAN+VAD vector costing less than 0.1\% of the model's parameters---reduce perplexity by 12.3\% over an unconditioned baseline trained on identical data.
The signal is cheap to compute (CPU encoders, {$<$}10\,ms) and the gain is reproducible.
...
\begin{enumerate}
    ...
    The 12.1\% improvement from adding conditioning quantifies the value of the social state for any downstream application.
...
\textbf{Current capability:} ConditionalDialogue achieves val\_ppl $=$ 2.90, with zero keyword-detected secret leakage in sampled generations.
The conditioning gain of 12.1\% over the unconditioned baseline is reliable and reproducible.
...
\begin{enumerate}
    ...
    If the state says ``secrecy\_pressure $=$ high, reveal\_decision $=$ none,'' the model is architecturally constrained to not leak sensitive information.
...
\midrule
Social-state-conditioned dialogue with 12.1\% PPL improvement
    & Track C: ConditionalDialogue (ppl $=$ 2.90) vs. TinyLlama SFT (ppl $=$ 3.30); zero secret leakage
