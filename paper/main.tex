...
\title{Structured Social State for Auditable NPC Dialogue Generation}
...
Training a multi-head predictor on Qwen3-4B reveals a systematic recoverability pattern: static attributes such as arousal (true Cohen's $\kappa{=}0.692$) and valence ($\kappa{=}0.688$) are highly predictable, while per-turn relational deltas are near chance (familiarity\_delta $\kappa{=}0.296$).
...
End-to-end routing with predicted $Z_t$ achieves F1$=$0.669; under our keyword-based heuristic detector, gated secret leakage is zero across evaluated turns. A 4-field decision-card compression of the full 29-head prompt improves policy consistency by 11.5 pp while maintaining zero contradictions.
A stratified audit of 150 test-set turns with five human annotators (all failing quality-control checks) shows that the social-state heads resist reliable uncalibrated human annotation; however, the best-effort annotators achieve moderate agreement with the teacher on valence ($\kappa{=}0.28$) and response policy ($\kappa{=}0.37$), while an AI validator aligns with the teacher on response policy ($\kappa{=}0.40$) and valence ($\kappa{=}0.32$), suggesting partial internal consistency in model space.
Familiarity level, repair strategy, and arousal remain non-auditable by either humans or models.
...
\begin{description}
    \item[RQ1] Can a pretrained LLM learn to predict the structured latent social state from dialogue context alone, and which dimensions are most recoverable?
    \item[RQ2] Does explicit social-state conditioning improve response generation quality, and is the gain attributable to semantic content or merely prefix capacity?
    \item[RQ3] What is the parameter--architecture trade-off for from-scratch SLMs on NPC dialogue at the 15--22M scale?
\end{description}

\paragraph{Contributions.}
(1)~A 29-dimensional NPC-specific social-state schema with formal cross-dimensional consistency constraints;
(2)~a recoverability analysis showing which dimensions are predictable from dialogue context (static attributes such as arousal and valence, true $\kappa{>}0.68$) and which are not (per-turn relational deltas, $\kappa{<}0.30$);
(3)~a placebo-controlled study showing that soft-prefix conditioning improves perplexity independently of the semantic content of the current OCEAN+VAD values, motivating structured social-state conditioning;
(4)~an end-to-end pipeline demonstrating that predicted $Z_t$ can route generation and gate secret leakage under a heuristic detector;
(5)~an A/B evaluation showing that a 4-field decision-card compression of the full 29-head state improves policy consistency by 11.5 pp (0.735$\to$0.850) while maintaining zero contradictions and reducing leakage.
...
We evaluate a Social-State JEPA auxiliary objective as an exploratory experiment (Appendix~\ref{app:jepa}) and report it as a mixed-to-null result across four backbones.

\subsection{Positioning Summary}

Appendix~\ref{app:positioning} summarises how our work differs from each prior line of research.

In summary, prior work has explored components of our system in isolation---NPC dialogue, social affect, conditioned generation, multi-head prediction---but, to our knowledge, no prior study has evaluated a fine-grained NPC-oriented social-state schema across recoverability, conditioning, and secrecy-aware generation within a single pipeline.
...
Gated secret leakage---the safety-relevant metric, computed only over turns where \texttt{reveal\_decision=none}---is zero under our keyword-based heuristic detector.
Ungated leakage across all 683 turns is 7.6\%, reflecting that the keyword-based detector flags some non-secret patterns; deployment-grade reliability would require manual verification or a trained classifier.
...
Test-set evaluation on 884 held-out turns (Table~\ref{tab:test_set}) confirms the validation results generalise with modest degradation: ROUGE-L drops from 0.145 to 0.139 ($-4.3\%$), Distinct-2 from 0.638 to 0.603, and length ratio from 0.91 to 0.86. Gated leakage remains zero on test under the same heuristic detector.
...
\subsection{Prompt Compression for Slow-Path Turns}
\label{sec:decision_card}

The slow path dumps the full 29-head $Z_t$ state into the prompt, which consumes $\sim$350 tokens and may overwhelm the response generator with irrelevant detail.
We therefore test a \textit{decision card}: a compressed instruction containing only stance summary, disclosure policy, risk level, and tone---four fields extracted from the predicted $Z_t$.
...
Policy consistency & 0.7349 & 0.8497 & $+$0.1148 & N/A \\
...
Avg generation length / ref & 29.3 / 32.2 & 23.7 / 32.2 & $-$5.6 tok & N/A \\
...
\textbf{Finding:} Compressing the slow-path prompt to a 4-field decision card improves policy consistency by \textbf{11.5 pp} (0.735$\to$0.850) while reducing secret leakage (gated 2.94\%$\to$2.45\%; ungated 4.98\%$\to$3.66\%) and maintaining zero contradictions (Table~\ref{tab:decision_card}).
The ROUGE-L drop ($-$0.047) is offset by shorter, more directive generations (length ratio 0.91$\to$0.73) that stay closer to the intended NPC policy.
This confirms that the full 29-head dump contains redundant information; a minimal, task-relevant subset is sufficient---and in this case beneficial---for controlled generation.

\subsection{Joint vs. Separate Training}
\label{sec:joint_vs_separate}

Appendix~Table~\ref{tab:joint_vs_separate} compares the separate-stage pipeline (Stage~1 latent predictor $+$ Stage~2 response generator) against the jointly trained Stage~3 model on per-head latent prediction metrics.
...
\paragraph{Social-State JEPA (exploratory).}
We also evaluated a JEPA auxiliary objective~\cite{lecun2022path,assran2023self} in which a predictor network learns to predict future $Z_{t+1}$ representations in embedding space.
Across four backbones, JEPA produced small aggregate $\kappa$ gains ($+0.012$ to $+0.027$) but more heads degraded than improved, and shuffled-future placebos partially replicated the effect.
No actionable head (response\_policy, reveal\_decision) showed a reliable improvement.
Full results appear in Appendix~\ref{app:jepa}.

\paragraph{Gold $Z_t$ response difficulty.}
Appendix~\ref{app:gold_zt_bridge} reports a per-field analysis with gold $Z_t$ conditioning.
Reveal decision has the largest effect on generation difficulty ($d{=}1.02$), while relational deltas have smaller effects ($d{<}0.50$), suggesting that the schema's most response-relevant information is concentrated in decision and conversational heads.
...
The interpretability of individual heads (e.g., ``\texttt{secrecy\_pressure=high}'' or ``\texttt{response\_policy=withhold}'') enables human auditing of NPC behaviour, though our stratified audit (Appendix~\ref{app:human_audit}) shows that only \texttt{valence} and \texttt{response\_policy} are reliably auditable by both humans and models; \texttt{familiarity\_level}, \texttt{repair\_strategy}, and \texttt{arousal} resist reliable annotation.
...
Gated secret leakage---computed only over turns where \texttt{reveal\_decision=none}---is zero under our keyword-based heuristic detector across all 683 validation turns.
...
This zero-detected-leakage result establishes the latent predictor's $D_t$ output as a promising mechanism for gating information disclosure, though deployment-grade reliability requires a validated classifier or human audit.
...
\textbf{Synthetic labels and human validation.}
All dialogue and social-state annotations are generated by a teacher LLM pipeline, not human annotators.
This introduces two concerns: the labels may reflect the teacher model's annotation style rather than human social judgments, and subjective dimensions such as relational stance and tone are particularly noisy.
To mitigate this, we conducted a stratified audit of 150 natural, non-counterfactual test-set turns across the most actionable heads, using five human annotators and one AI validator.
All five annotators failed post-hoc quality-control checks (speed-running, default-option bias, or timer abuse), and human--human agreement is near zero for most heads and most pairs.
The AI validator agrees more strongly with the teacher on \texttt{response\_policy} ($\kappa{=}0.40$) and \texttt{valence} ($\kappa{=}0.32$), and the best-effort human annotators corroborate this on \texttt{valence} ($\kappa{=}0.69$--$0.76$ vs.\ AI) and \texttt{response\_policy} ($\kappa{=}0.49$--$0.53$ vs.\ AI).
However, \texttt{familiarity\_level}, \texttt{repair\_strategy}, and \texttt{arousal} show near-zero agreement on all axes and are classified as non-auditable.
Relational deltas remain unvalidated and are especially noisy in model evaluation.
Full agreement statistics, quality-control details, and the three-tier audibility classification are reported in Appendix~\ref{app:human_audit}.

\textbf{Small data scale.}
The full dataset comprises 7,742 turns across 736 episodes.
While sufficient to train a multi-head predictor and a response generator, this is small by modern NLP standards, and results---especially for rare decision classes---may shift with an order-of-magnitude increase in data.

\textbf{Weak automatic dialogue metrics.}
BLEU, ROUGE-L, and Distinct are well-known to be weak proxies for dialogue quality, role consistency, and social appropriateness.
Our reported gains in perplexity and lexical diversity are meaningful for model development, but deployment-ready claims about NPC response quality require human preference evaluation or task-success measurement.

\textbf{Heuristic leakage detection.}
The secret-leakage detector is keyword-based and has not been validated for precision or recall against a manually annotated sample.
The ``zero gated leakage'' result therefore means zero detections under this heuristic, not zero leakage in principle.
A trained classifier or manual verification is needed before deploying the gating mechanism in production.

\textbf{Limited generalization.}
All scenarios are drawn from a single fantasy-narrative setting (``Oakhaven Siege'') with seven template types.
Generalization to modern settings, open-world dialogue, or non-English languages is untested.

\textbf{Joint-model comparison.}
The three-stage Qwen3-4B joint model has been trained with consistency loss, but paired statistical comparison against the separate-stage pipeline shows a small mean degradation ($\Delta\kappa{=}-0.024$) rather than improvement, and per-head trade-offs require formal significance testing.
...
Figure~\ref{fig:data_flow} shows the complete flow from scenario generation through training to evaluation.
...
To validate the synthetic social-state labels, we conducted a stratified audit of 150 natural, non-counterfactual test-set turns (approximately 21 per scenario type) drawn from a cleaned 356-turn audit packet.
...
We also ran an AI validator (Gemma-4-31B-IT, zero-shot) on the same 150 turns as a model-based third judge; this is reported separately from human--human agreement and is not treated as a replacement for human validation.

\subsection{Quality-Control Screening}

All five annotators failed post-hoc quality-control checks (Table~\ref{tab:audit_qc}).
...
\subsection{AI Validator vs.\ Teacher (Internal Consistency)}

Table~\ref{tab:ai_teacher} reports agreement between the AI validator and the teacher labels.
...
The AI validator achieves moderate agreement with the teacher on \texttt{response\_policy} ($\kappa{=}0.40$) and \texttt{valence} ($\kappa{=}0.32$), confirming that these heads carry recoverable signal that is consistent across independently generated model outputs.
Heads with low AI--teacher agreement---\texttt{trust\_level} ($\kappa{=}0.02$), \texttt{familiarity\_level} ($\kappa{=}0.02$), and \texttt{arousal} ($\kappa{=}0.05$)---indicate dimensions where even models disagree, suggesting inherent ambiguity or underspecification in the labelling guidelines.

\begin{table}[h]
\centering
\caption{AI validator vs.\ teacher labels ($n{=}150$ turns). Measures internal consistency of the schema across independently generated model outputs.}
\label{tab:ai_teacher}
\small
\begin{tabular}{@{}lrr@{}}
\toprule
\textbf{Head} & \textbf{AI--Teacher Acc} & \textbf{AI--Teacher $\kappa$} \\
\midrule
valence              & 0.527 & 0.316 \\
arousal              & 0.367 & 0.045 \\
...
Even the best human annotator, however, achieves $\kappa{<}0.10$ on \texttt{familiarity\_level}, \texttt{repair\_strategy}, and \texttt{arousal}, confirming that these heads resist reliable human annotation under the current protocol.
...
Human--human agreement is near zero or negative for most heads and most pairs, confirming that the eight-head annotation task is difficult for uncalibrated annotators.
...
\subsection{Human--AI Validator Agreement}

Table~\ref{tab:human_ai} reports agreement between each human annotator and the AI validator.
The best-effort annotators (H4, H5) achieve moderate-to-substantial agreement with the AI validator on \texttt{valence} ($\kappa{=}0.69$ and $0.76$), \texttt{response\_policy} ($\kappa{=}0.49$ and $0.53$), and \texttt{reveal\_decision} ($\kappa{=}0.44$ and $0.46$), suggesting that these heads capture signal that is consistent across human and model judgement.
In contrast, \texttt{familiarity\_level} and \texttt{repair\_strategy} show near-zero or negative human--AI $\kappa$ for all annotators, confirming these as the most ambiguous heads.

\begin{table}[h]
\centering
\caption{Per-annotator human--AI validator agreement ($\kappa$). H1--H4 share 150 turns with the AI validator; H5 shares 82.}
\label{tab:human_ai}
\small
\begin{tabular}{@{}lrrrrr@{}}
\toprule
...
Table~\ref{tab:audit_summary} consolidates the three agreement axes---human--teacher (best annotator), human--human (range across pairs), and AI--teacher---into a single per-head summary.
Heads fall into three tiers:

\begin{enumerate}
    \item \textbf{Partially auditable:} \texttt{valence} and \texttt{response\_policy} achieve the highest agreement across all axes, with best-human--teacher $\kappa{\geq}0.27$, maximum HH $\kappa{\geq}0.40$, and AI--teacher $\kappa{\geq}0.32$.
...
\begin{table}[h]
\centering
\caption{Consolidated audit summary per head. Best HT $\kappa$ = highest human--teacher $\kappa$ across five annotators. HH $\kappa$ range = min--max across all pairs with $\geq$50 common turns. AI-T $\kappa$ = AI validator vs.\ teacher.}
\label{tab:audit_summary}
\small
\begin{tabular}{@{}lrrrr@{}}
\toprule
\textbf{Head} & \textbf{Best HT $\kappa$} & \textbf{HH $\kappa$ range} & \textbf{AI-T $\kappa$} & \textbf{Tier} \\
\midrule
valence              & 0.280 & $-$0.09\,--\,0.79 & 0.316 & Partially auditable \\
response\_policy     & 0.374 & $-$0.05\,--\,0.42 & 0.397 & Partially auditable \\
...
arousal              & 0.107 & $-$0.17\,--\,0.32 & 0.045 & Non-auditable \\
repair\_strategy     & 0.085 & $-$0.03\,--\,0.02 & 0.136 & Non-auditable \\
familiarity\_level   & 0.021 & $-$0.01\,--\,0.06 & 0.019 & Non-auditable \\
\bottomrule
\end{tabular}
\end{table}
...
Human--human $\kappa$ is near zero for most heads and most annotator pairs, and all five annotators failed post-hoc quality-control checks for speed, default-option bias, or timer abuse.
The AI validator agrees more strongly with the teacher on \texttt{response\_policy} ($\kappa{=}0.40$) and \texttt{valence} ($\kappa{=}0.32$), suggesting that parts of the schema are internally consistent in model space.
The best-effort human annotators (H4, H5) corroborate this: they achieve moderate-to-substantial agreement with the AI validator on \texttt{valence} ($\kappa{=}0.69$--$0.76$) and \texttt{response\_policy} ($\kappa{=}0.49$--$0.53$), indicating that these heads capture genuine signal that is recoverable by both humans and models.
However, \texttt{familiarity\_level}, \texttt{repair\_strategy}, and \texttt{arousal} resist reliable annotation by either humans or the AI validator, suggesting inherent ambiguity or underspecification in the current labelling guidelines.
