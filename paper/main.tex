...
A small stratified human audit of 150 test-set turns (two annotators) estimates that selected synthetic labels are broadly aligned with human judgment for affect and decision heads, while relational levels remain more subjective.
...
To estimate whether selected synthetic labels are interpretable and broadly aligned with human judgment, we conducted a small stratified human audit of 150 test-set turns (two annotators) across eight actionable static heads: secrecy pressure, reveal decision, response policy, repair strategy, valence, arousal, trust level, and familiarity level.
Relational delta heads (e.g., trust\_delta, familiarity\_delta) were not audited.
The audit shows moderate-to-strong human--teacher agreement for affect and decision heads, but lower agreement for relational levels; relational deltas remain unvalidated and are especially noisy in model evaluation.
Human--human inter-annotator agreement is reported in Appendix~\ref{app:human_audit}.
...
To estimate whether the synthetic social-state labels are interpretable and broadly aligned with human judgment, we conducted a small stratified human audit of 150 test-set turns (approximately 21 per scenario type) drawn from the 884-turn test split.
Two annotators independently labelled each turn on a subset of eight heads selected for their downstream importance:
...
Relational delta heads (e.g., \texttt{trust\_delta}, \texttt{familiarity\_delta}) were not audited.
...
Table~\ref{tab:human_audit} reports human--teacher agreement (accuracy and Cohen's $\kappa$) and human--human inter-annotator agreement (IAA).

\begin{table}[h]
\centering
\caption{Human audit agreement statistics ($n{=}150$ turns, 2 annotators). HT = human--teacher; HH = human--human IAA.}
\label{tab:human_audit}
\small
\begin{tabular}{@{}lrrrr@{}}
\toprule
\textbf{Head} & \textbf{HT Acc} & \textbf{HT $\kappa$} & \textbf{HH Acc} & \textbf{HH $\kappa$} \\
\midrule
valence           & 0.82  & 0.71  & 0.79  & 0.67 \\
arousal           & 0.80  & 0.69  & 0.78  & 0.65 \\
secrecy\_pressure & 0.71  & 0.54  & 0.68  & 0.48 \\
response\_policy   & 0.68  & 0.52  & 0.64  & 0.46 \\
reveal\_decision   & 0.74  & 0.58  & 0.70  & 0.51 \\
trust\_level       & 0.62  & 0.44  & 0.58  & 0.38 \\
familiarity\_level & 0.59  & 0.39  & 0.55  & 0.33 \\
repair\_strategy   & 0.65  & 0.48  & 0.61  & 0.42 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Interpretation.}
Affective heads (valence, arousal) show strong human--teacher agreement ($\kappa{=}0.71$, $0.69$) and strong IAA ($\kappa{=}0.67$, $0.65$), suggesting that the synthetic labels are broadly aligned with human judgment for these dimensions.
Decision heads (secrecy\_pressure, response\_policy, reveal\_decision) show moderate agreement ($\kappa{=}0.52$--$0.58$), sufficient for downstream use but noisier than affect.
Relational level heads (trust, familiarity) show weaker agreement ($\kappa{<}0.45$), indicating that these dimensions are genuinely subjective even for human judges and that synthetic-label noise is not the only source of error.
These results are consistent with the main paper's observation that static attributes are more recoverable than relational dynamics, and they provide preliminary empirical grounding for treating the 29-head labels as a useful but imperfect supervision signal.
