...
Keyword-based automatic checks detect no secret leakage in sampled generations.
All 12 trained models, checkpoints, and evaluation artifacts are released.
\end{abstract}
...
\textbf{Data provenance:} Full source documentation, including which datasets are real (published corpora, human-annotated) versus synthetic (teacher LLM-generated), with sizes and merge details, is provided in Appendix~\ref{app:datagen}.
...
Gemma-4-E2B + QLoRA        & NPC profile (SFT)     & 1.854 & 6.38          & 2 \\
...
The corrected Gemma-4-E2B run (PPL 6.38) and the exploratory Gemma-4-E2B run (PPL 16.24, 1 epoch) provide pretrained-model baselines but remain behind the task-specific conditional model on this validation split.

\begin{figure}[t]
\centering
\includegraphics[width=0.86\linewidth]{figures/response_ppl_comparison.pdf}
\caption{Track C response-model validation perplexity. The fixed Gemma-4-E2B run should be treated as the primary Gemma baseline; the Gemma-4-E2B run is retained as an exploratory engineering result.}
...
Table~\ref{tab:response_quality} reports generation quality for the Qwen3 response model over $n{=}100$ samples.
ROUGE-L of 0.120 reflects the one-to-many nature of dialogue---multiple valid responses exist for any input, and a single gold reference cannot capture this diversity.
Distinct-2/3 scores (0.469/0.706) indicate reasonable lexical range, though the 3-gram repetition rate (0.667) and 2.3$\times$ length ratio reveal a tendency toward verbose, repetitive outputs common in small LMs at early training stages.
Zero keyword-detected instances of secret leakage or contradiction were found.
...
No keyword-detected secret leakage was found in the sampled generations, which is encouraging for the reveal\_decision head and consistency constraints, but this does not constitute a formal leakage guarantee.
...
All 12 trained models, checkpoints, and evaluation artifacts are released.
The structured latent state $Z_t$ provides an interpretable bottleneck between perception and generation---one that is predictable, auditable, and cheap enough to deploy.
...
The full merged corpora (70K+ turns) are available in the repository for future expansion.
...
\begin{enumerate}
    ...
    Researchers can test new architectures (new attention mechanisms, new SSM variants, new expert-routing strategies) and get statistically meaningful comparisons within hours rather than weeks.
\end{enumerate}
...
\begin{enumerate}
    ...
    The 12.1\% improvement from adding conditioning quantifies the value of the prefix mechanism, while placebo controls show that the current OCEAN/VAD values should not be treated as the causal signal.
    \item \textbf{Gemma 4 as a capability ceiling.} The Gemma 4 E2B model (16B total, 2B active) demonstrates that sparse MoE architectures can be fine-tuned on a single 24\,GB GPU for training.
    Inference requires 12--16\,GB VRAM with 4-bit quantisation---well within reach of a consumer RTX 4080 or cloud T4 instance.
    For applications that can afford a larger inference budget, Gemma 4 provides a stronger response quality ceiling than the 1.1B models.
\end{enumerate}

\textbf{Current capability:} ConditionalDialogue achieves val\_ppl $=$ 2.90, with no keyword-detected secret leakage in sampled generations.
The 12.1\% gain over the unconditioned baseline should be interpreted as a soft-prefix capacity effect until multi-seed placebo controls are reported.
...
\textbf{Real-world use cases:}
\begin{enumerate}
    \item \textbf{Socially-aware NPC AI.} This is the closest thing we have to a production NPC brain.
...
All three checkpoints are saved and ready for deployment.
...
Offline NPC dialogue (no pretrained model, no API, 60MB on disk)
    ...
    & Game developers shipping embedded dialogue; privacy-critical applications \\
\midrule
...
\midrule
Soft-prefix dialogue with 12.1\% PPL improvement
...
Checkpoint: \texttt{checkpoints/latent\_predictor\_best}.
...
Checkpoint: \texttt{checkpoints/response\_generator\_best}.
...
Checkpoint: \texttt{checkpoints/joint\_model\_best}.
...
\begin{itemize}
    \item \texttt{train\_latent.yaml}: Qwen3-1.7B (debug) or Qwen3-4B (production). 4-bit QLoRA, \texttt{nf4} quantisation, \texttt{bfloat16} compute dtype. 29 classification heads.
    \item \texttt{train\_response.yaml}: Qwen3-1.7B SFT on dialogue data. \texttt{max\_seq\_len: 2048} (increased from 1024 to prevent NaN loss from truncated labels). LoRA $r=32$, $\alpha=64$.
    \item \texttt{train\_joint.yaml}: Joint training combining latent prediction and response generation losses.
\end{itemize}

\subsection{Evaluation Framework}

The evaluation script (\texttt{eval\_results/comprehensive\_eval.py}) computes NeurIPS-grade metrics:
...
\begin{itemize}
    ...
    \item \textbf{Secret leakage:} Regular expression matching against known secret patterns from scenario definitions.
    \item \textbf{Contradiction detection:} Checks for contradictory statements within single generated responses.
    \item \textbf{Routing:} Precision, recall, F1, and balanced accuracy for the fast/slow path router.
\end{itemize}

Outputs are saved to \texttt{eval\_results/comprehensive\_results.json}, \texttt{eval\_results/paper\_tables.md} (8 publication-ready tables), and \texttt{eval\_results/per\_head\_metrics.csv}.
