...
Calls OpenRouter to annotate the same 150 stratified dialogue turns on the same
8 heads as human annotators. This produces a third-validator JSONL file that is
compatible with compute_audit_agreement.py and audit_quality_control.py.

Usage:
    uv run ai_audit.py --data ./audit_input_clean.jsonl --output ./audit_results
    uv run ai_audit.py --data ./audit_input_clean.jsonl --output ./audit_results --sample-size 5  # quick test
"""
...
def stratify_sample(records: list[dict], n: int = SAMPLE_SIZE, seed: int = 42) -> list[dict]:
    """Draw the same deterministic stratified sample as human_audit_app.py."""
...
# ---------------------------------------------------------------------------
def call_openrouter(api_key: str, prompt: str, model: str, max_retries: int = 5) -> dict:
    headers = {
...
def main():
    ...
    parser.add_argument("--data", required=True, type=Path, help="Path to audit_input_clean.jsonl")
    ...
    parser.add_argument("--seed", type=int, default=42, help="Sampling seed; keep 42 to match human audit")
    parser.add_argument("--model", default=MODEL, help="OpenRouter model id")
    parser.add_argument("--annotator", default="ai_validator", help="Annotator id to write into the JSONL")
    ...
    parser.add_argument("--resume", action="store_true", help="Skip turn_ids already present in the output file")
    ...
    turns = stratify_sample(records, n=args.sample_size, seed=args.seed)
    print(f"Loaded {len(turns)} turns from {args.data}")

    args.output.mkdir(parents=True, exist_ok=True)
    out_path = args.output / f"audit_{args.annotator}.jsonl"
    meta_path = args.output / f"audit_{args.annotator}_meta.json"

    completed: set[str] = set()
    if args.resume and out_path.exists():
        for rec in load_data(out_path):
            tid = rec.get("turn_id")
            if tid:
                completed.add(tid)
        if completed:
            print(f"Resume enabled: skipping {len(completed)} existing annotations in {out_path}")
    ...
    for i, turn in enumerate(turns):
        ...
        try:
            result = call_openrouter(api_key, prompt, model=args.model, max_retries=args.max_retries)
    ...
    meta = {
        ...
        "annotated_count": len(annotations) + len(completed),
