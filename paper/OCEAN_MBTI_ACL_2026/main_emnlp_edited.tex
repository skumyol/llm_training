...
This operating point is conservative: the router sends many borderline turns to the slow path where generation is constrained.
...
Leakage. Under the keyword-based heuristic detector, no gated secret leakage is detected in the evaluated turns.
...
\begin{table}[t]
\centering
\caption{Routing performance with predicted latent states (Qwen3-4B). Routing uses the best validation checkpoint; single-seed deployment-style evaluation.}
\label{tab:routing}
\small
\begin{tabularx}{\linewidth}{@{}Xr@{}}
\toprule
...
The predicted-state pipeline runs end-to-end (routing F1$=$0.669; ROUGE-L$=$0.145; zero detected gated leakage under the keyword heuristic), but these metrics should be read as development diagnostics.
...
The dataset is also small and domain-specific: 7,742 turns across 736 episodes in the fantasy setting ``Oakhaven Siege''. Results may change with larger corpora, different genres, open-world dialogue, or non-English settings, especially for rare decision classes.
