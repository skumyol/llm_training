...
A stratified audit of 150 natural test-set turns shows that several social-state heads are difficult for uncalibrated human annotators, with low human--human agreement across most audited heads.
A model-based third validator aligns more strongly with the teacher on response policy and valence, suggesting partial internal consistency but not human-grounded gold labels.
...
To mitigate this, we conducted a stratified audit of 150 natural, non-counterfactual test-set turns across the most actionable heads (secrecy pressure, reveal decision, response policy, repair strategy, valence, arousal, trust level, familiarity level), using two human annotators and one AI validator.
The audit shows that the label schema should be treated as teacher-generated supervision rather than ground-truth human social cognition.
Human--human agreement is low across most audited heads, while the AI validator aligns more strongly with the teacher on response policy and valence.
Relational deltas remain unvalidated and are especially noisy in model evaluation.
Full agreement statistics are reported in Appendix~\ref{app:human_audit}.
...
Two human annotators independently labelled each turn on a subset of eight heads selected for their downstream importance:
\texttt{secrecy\_pressure}, \texttt{reveal\_decision}, \texttt{response\_policy}, \texttt{repair\_strategy}, \texttt{valence}, \texttt{arousal}, \texttt{trust\_level}, and \texttt{familiarity\_level}.
...
We also ran an AI validator on the same 150 turns as a model-based third judge; this is reported separately from human--human agreement and is not treated as a replacement for human validation.

\subsection{Agreement Results}

Table~\ref{tab:human_audit} reports human--teacher agreement (accuracy and Cohen's $\kappa$, averaged across the two humans), human--human inter-annotator agreement (IAA), and AI--teacher agreement.

\begin{table}[h]
\centering
\caption{Audit agreement statistics ($n{=}150$ turns). HT = human--teacher averaged across two humans; HH = human--human IAA; AI-T = AI validator vs. teacher.}
\label{tab:human_audit}
\small
\begin{tabular}{@{}lrrrrrr@{}}
\toprule
\textbf{Head} & \textbf{HT Acc} & \textbf{HT $\kappa$} & \textbf{HH Acc} & \textbf{HH $\kappa$} & \textbf{AI-T Acc} & \textbf{AI-T $\kappa$} \\
\midrule
...
The human audit does not support treating the synthetic labels as human-validated gold annotations: human--human $\kappa$ is near zero for most heads.
The AI validator agrees more strongly with the teacher on response policy ($\kappa{=}0.40$) and valence ($\kappa{=}0.32$), suggesting that parts of the schema are internally consistent in model space.
However, the weak human--human agreement indicates that the annotation protocol needs clearer calibration examples and adjudication before these labels can be claimed as human-grounded supervision.
