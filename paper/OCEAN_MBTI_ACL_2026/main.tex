...
A stratified human audit of 150 test-set turns confirms moderate agreement with synthetic labels for the most actionable heads.
The audit supports the labels as useful but imperfect supervision signals, with stronger reliability for affect and decision heads than for relational stance.
...
To mitigate this, we conducted a stratified human audit of 150 test-set turns (two annotators) across the most actionable heads (secrecy pressure, reveal decision, response policy, valence, arousal, trust level, familiarity level).
The audit validates the label schema as a useful supervision signal, not as ground-truth human social cognition.
It shows moderate-to-strong agreement with synthetic labels for affect and decision heads, but lower agreement for relational level heads; relational deltas remain unvalidated and are especially noisy in model evaluation.
...
To validate the synthetic social-state labels, we conducted a stratified human audit of 150 test-set turns (approximately 21 per scenario type) drawn from the 884-turn test split.
...
Affective heads (valence, arousal) show strong human--teacher agreement ($\kappa{=}0.71$, $0.69$) and strong IAA ($\kappa{=}0.67$, $0.65$), confirming that the synthetic labels are credible for these dimensions.
...
These results support the main paper's claim that static attributes are more recoverable than relational dynamics, and they provide empirical grounding for treating the 29-head labels as a useful but imperfect supervision signal.
